...
it.effect("always grants pull-requests and gates browser and device access independently", () =>
  Effect.gen(function* () {
    ...
    expect(yield* capabilitiesOf(withPreview)).toEqual(["preview", "pull-requests", "tasks"]);
    expect(yield* capabilitiesOf(withoutPreview)).toEqual(["pull-requests", "tasks"]);
    expect(yield* capabilitiesOf(withDevice)).toEqual(["device", "pull-requests", "tasks"]);
  }),
);
...
it.effect("task access does not grant browser access", () =>
  Effect.gen(function* () {
    const registry = yield* makeRegistry(() => 1000);
    const issued = yield* registry.issue({
      threadId: ThreadId.make("task-only"),
      providerInstanceId: ProviderInstanceId.make("codex"),
      capabilities: new Set(),
    });
