...
it.layer(NodeSqliteClient.layer({ filename: ":memory:" }))(
  "nightly upgrade from fork migrations",
  (it) => {
    it.effect("preserves fork origins and unsent context while adding nightly columns", () =>
      ...
      `;
        assert.deepEqual(threads, [
          {
            fork_source_thread_id: "source",
            fork_source_message_id: "message",
            fork_source_sequence: 12,
...
it.layer(NodeSqliteClient.layer({ filename: ":memory:" }))("September 20 fork upgrade", (it) => {
  it.effect(
    "preserves the shipped schema and data while appending all four nightly migrations",
    () =>
      Effect.gen(function* () {
        const sql = yield* SqlClient.SqlClient;
        yield* runMigrations({ toMigrationInclusive: 51 });
        const now = "2026-09-19T12:00:00.000Z";
        // @effect-diagnostics-next-line preferSchemaOverJson:off - hand-written legacy row fixture.
