...
describe("ProviderCommandReactor", () => {
  ...
  effectIt.effect("retains a turn dispatched immediately after start until activation", () =>
    Effect.gen(function* () {
      ...
      expect(harness.sendTurn.mock.calls[0]?.[0]).toMatchObject({
        ...
        messageTime: { submittedAt: "2026-01-01T00:00:00.000Z" },
      });
    }),
  );
  ...
  describe("forked thread first-turn context injection", () => {
    async function seedForkedThread(
      ...
      await harness.runEffect(
        Effect.gen(function* () {
          ...
          const capturedChars = input.entries.reduce(
    ...
    effectIt.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          // The consumption mark and activity append run inside the sendTurn call's
          ...
          yield* Effect.promise(() =>
            waitFor(
              async () => Option.getOrUndefined(await getForkContextRow())?.consumedAt != null,
            ),
          );
    ...
    effectIt.effect("restores consumed fork context when its first turn has been rewound", () =>
      Effect.gen(function* () {
        const harness = yield* Effect.promise(() => createHarness());
        const threadId = ThreadId.make("thread-forked-rewound");
        yield* Effect.promise(() =>
          seedForkedThread(harness, {
            childThreadId: threadId,
            entries: [{ kind: "assistant", text: "Keep the captured alpha requirement." }],
          }),
        );
        yield* Effect.promise(() =>
          harness.runEffect(
            Effect.gen(function* () {
              const repository = yield* ThreadForkContextRepository;
              yield* repository.markConsumed({ threadId, consumedAt: "2026-01-01T00:00:00.000Z" });
            }),
          ),
        );
        ...
        harness.sendTurn.mockImplementation(() =>
          Deferred.succeed(sent, undefined).pipe(
            Effect.as({ threadId, turnId: asTurnId("turn-rewound") }),
          ),
        );
        yield* harness.engine.dispatch({
          type: "thread.turn.start",
          commandId: CommandId.make("cmd-forked-after-rewind"),
          threadId,
          message: {
            messageId: asMessageId("forked-user-after-rewind"),
            role: "user",
        ...
        expect(harness.sendTurn.mock.calls[0]?.[0]).toMatchObject({
          input: expect.stringContaining("Keep the captured alpha requirement."),
        });
      }),
    );
    ...
    effectIt.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          // Marking consumption runs inside the sendTurn call's forked scope after it
          ...
          yield* Effect.promise(() =>
            waitFor(async () => Option.getOrUndefined(await getRow())?.consumedAt != null),
          );
        }),
    );
