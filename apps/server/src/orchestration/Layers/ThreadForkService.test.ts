...
const createSystem = Effect.gen(function* () {
  ...
  return { engine, snapshotQuery, threadForkService, forkContextRepository };
});

/** Seeds a project and a source thread with one user + one settled assistant message. */
const seedSourceThread = Effect.fn("seedSourceThread")(function* (
  system: Effect.Success<typeof createSystem>,
...
describe("ThreadForkService", () => {
  it.effect("creates a child thread with forkedFrom and stores the fork context row", () =>
    Effect.gen(function* () {
      const system = yield* createSystem;
      const { assistantMessageId } = yield* seedSourceThread(system, {
        projectId: "project-1",
        threadId: "thread-source",
        assistantText: "Here is the answer.",
      });

      const result = yield* system.threadForkService.forkThread(
        forkInput({
      ...
      const snapshot = yield* system.snapshotQuery.getSnapshot();
      ...
      const row = yield* system.forkContextRepository.get({
  ...
  it.effect("rejects a fork whose source message does not exist", () =>
    Effect.gen(function* () {
      const system = yield* createSystem;
      yield* seedSourceThread(system, {
        projectId: "project-1",
        threadId: "thread-source",
        assistantText: "Here is the answer.",
      });

      const error = yield* system.threadForkService
        .forkThread(
          forkInput({
            threadId: "thread-child",
            sourceThreadId: "thread-source",
  ...
  it.effect("rejects a fork whose boundary message is not an assistant message", () =>
    Effect.gen(function* () {
      const system = yield* createSystem;
      yield* seedSourceThread(system, {
        projectId: "project-1",
  ...
  it.effect("never stores a fork context row when the create is rejected", () =>
    Effect.gen(function* () {
      const system = yield* createSystem;
      const { assistantMessageId } = yield* seedSourceThread(system, {
        projectId: "project-1",
  ...
  it.effect("gives the post-dispatch rejection its own message text", () =>
    Effect.gen(function* () {
      const system = yield* createSystem;
      const { assistantMessageId } = yield* seedSourceThread(system, {
        projectId: "project-1",
  ...
  it.effect("bounds captured entries to the send-time limit, dropping the oldest first", () =>
    Effect.gen(function* () {
      const system = yield* createSystem;
      yield* system.engine.dispatch({
        type: "project.create",
      ...
      const row = yield* system.forkContextRepository.get({
        threadId: ThreadId.make("thread-child-bounds"),
      });
      expect(Option.isSome(row)).toBe(true);
      if (Option.isSome(row)) {
  ...
  it.effect("keeps the boundary entry even when it alone exceeds the capture limit", () =>
    Effect.gen(function* () {
      const system = yield* createSystem;
      yield* system.engine.dispatch({
        type: "project.create",
