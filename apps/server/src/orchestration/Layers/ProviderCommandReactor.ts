...
const make = Effect.gen(function* () {
  ...
  const processTurnStartRequested = Effect.fn("processTurnStartRequested")(function* (
    ...
    const isFirstUserMessageTurn = !hasOtherUserMessages && !isCompactCommand;
    ...
    // fork row is marked consumed and the `fork.context-sent` activity is appended
    ...
    let forkContextConsumption:
      ...
      | undefined;
    let sendTurnMessageText = projectComposerContextForProvider({
      text: message.text,
    ...
    if (!isCompactCommand && (isFirstUserMessageTurn || thread.forkedFrom)) {
      const project = yield* resolveProject(thread.projectId);
      generationCwd =
        resolveThreadWorkspaceCwd({
          thread,
    ...
    // Retain the snapshot for first-turn rewind. Failed sends remain unconsumed.
    if (thread.forkedFrom && !isCompactCommand) {
      const forkRowOption = yield* threadForkContextRepository.get({ threadId: thread.id }).pipe(
        Effect.catchCause((cause) =>
      ...
      if (
        Option.isSome(forkRowOption) &&
        (forkRowOption.value.consumedAt === null || !turnStart.value.previousUserMessageAt)
      ) {
        ...
        const citationExpandedUserText = expandAssistantCitationsForProvider(sendTurnMessageText);
    ...
    const previousUserMessageAt = turnStart.value.previousUserMessageAt;
    ...
    const send = providerService.sendTurn(sendTurnRequest.value).pipe(
      Effect.tap((sentTurn) => {
        ...
        return Effect.gen(function* () {
          yield* threadForkContextRepository.markConsumed({
            threadId: thread.id,
