...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  /** Attach task tools and, when permitted, browser tools to the provider session. */
  /**
  ...
  const prepareMcpSession = (threadId: ThreadId, providerInstanceId: ProviderInstanceId) =>
    Effect.gen(function* () {
      ...
      // Revoke the previous credential before reissuing a tasks-only session when browser access is off.
      if (!browserAccess) yield* revokeMcpCredential(threadId);
      const credential = yield* issueMcpCredential({ threadId, providerInstanceId, browserAccess });
      if (credential) {
  ...
  const sendTurn: ProviderServiceMethod<"sendTurn"> = Effect.fn("sendTurn")(function* (rawInput) {
    ...
    // ponytail: a prompt at the provider cap omits time context; use native metadata if providers expose it.
    appendAttachmentContext(
      parsed.messageTime ? formatMessageTime(parsed.messageTime, yield* nowIso) : undefined,
    );
