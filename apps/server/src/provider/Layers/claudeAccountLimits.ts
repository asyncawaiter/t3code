/**
 * On-demand Claude Code account limits. When the SDK cannot report limits,
 * this reads the same claude.ai usage endpoint the CLI's `/usage`
...
export function claudeAccountUsageSnapshot(
  update: UsageLimitsUpdate,
  checkedAt: string,
): ServerProviderUsageLimits {
  return {
    checkedAt,
    windows: update.windows.map(({ windowMinutes, resetsAt, ...window }) => ({
      ...window,
      kind: windowMinutes === 300 ? "session" : windowMinutes === 10080 ? "weekly" : "other",
      ...(windowMinutes === null ? {} : { windowDurationMins: windowMinutes }),
      ...(resetsAt && Number.isFinite(Date.parse(resetsAt)) ? { resetsAt } : {}),
    })),
  };
}
