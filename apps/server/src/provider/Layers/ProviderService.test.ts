...
describe("agent browser access", () => {
  ...
  const startSessionWith = (
    ...
    Effect.gen(function* () {
      const issued: Array<{ threadId: ThreadId; browserAccess: boolean | undefined }> = [];
      const codex = makeFakeCodexAdapter();
      const providerAdapterLayer = Layer.succeed(
        ProviderAdapterRegistry.ProviderAdapterRegistry,
      ...
      const providerLayer = makeProviderServiceLive({
        issueMcpCredential: (request) =>
          Effect.sync(() => {
            issued.push({ threadId: request.threadId, browserAccess: request.browserAccess });
            return undefined;
          }),
  ...
  // Task tools remain available independently of browser access.
  it.effect("requests a tasks-only credential when agent browser access is off", () =>
    Effect.gen(function* () {
      const issued = yield* startSessionWith(false, asThreadId("thread-browser-off"));

      assert.deepEqual(issued, [
        { threadId: asThreadId("thread-browser-off"), browserAccess: false },
      ]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("requests an MCP credential when agent browser access is on", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [{ threadId, browserAccess: true }]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );

  it.effect(
    "replaces the credential with tasks-only access when the project disables browser access",
    () =>
      Effect.gen(function* () {
        const threadId = asThreadId("thread-project-browser-off");
        revokedThreads.length = 0;
        const issued = yield* startSessionWith(true, threadId, false);
        assert.deepEqual(issued, [{ threadId, browserAccess: false }]);
        assert.deepEqual(revokedThreads, [threadId]);
      }).pipe(Effect.provide(NodeServices.layer)),
  );
